...
RUN apt-get update \
  ...
  libevent-dev libopus-dev  libpcap-dev libxmlrpc-core-c3-dev markdown \
  libjson-glib-dev default-libmysqlclient-dev libhiredis-dev libssl-dev \
  libcurl4-openssl-dev libavcodec-extra gperf libspandsp-dev libwebsockets-dev pandoc libjson-glib-1.0-0\
  && cd /usr/local/src \
